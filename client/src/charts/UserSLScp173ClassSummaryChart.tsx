...
const UserSLScp173ClassSummaryChart = ({userSlScpClassOverview}:any) => {
    return (
        ...
        <div className="sm:bg-dark_opacity grid md:grid-cols-3 md:grid-rows-1 sm:grid-cols-2 sm:grid-rows-2 gap-[33px] sm:gap-0">
          <div className=" h-[330px] bg-dark_opacity sm:bg-transparent grid grid-cols-1 grid-rows-2">
            <div className="flex flex-col justify-center items-center border-b-[1px] border-white_opacity">
              <p className="text-[48px]">{!userSlScpClassOverview[0].roleStats.kdRatio ? userSlScpClassOverview[0].roleStats.kills : userSlScpClassOverview[0].roleStats.kdRatio }</p>
            ...
            <div className="grid grid-cols-3 grid-rows-2">
              <div className="flex flex-col justify-center items-center border-r-[1px] border-b-[1px] border-white_opacity">
                <p className="text-[18px] xs:text-[20px]">{userSlScpClassOverview[0].roleStats.kills}</p>
                <p className="text-[11px] xs:text-[14px]">KILLS</p>
              </div>
              <div className="flex flex-col justify-center items-center border-b-[1px] border-white_opacity">
                <p className="text-[18px] xs:text-[20px]">{userSlScpClassOverview[0].roleStats.deaths}</p>
              ...
              <div className="flex flex-col justify-center items-center border-l-[1px] border-b-[1px] border-white_opacity">
                <p className="text-[18px] xs:text-[20px]">{userSlScpClassOverview[0].roleStats.timesJumped }</p>
              ...
              <div className="flex flex-col justify-center items-center border-r-[1px] border-white_opacity">
                <p className="text-[18px] xs:text-[20px]">{(userSlScpClassOverview[0].roleStats.kills/(userSlScpClassOverview[0].roleStats.timePlayed/60)).toFixed(2)}</p>
                <p className="text-[11px] xs:text-[14px]">KPM</p>
              </div>
              <div className="flex flex-col justify-center items-center ">
                <p className="text-[18px] xs:text-[20px]">{(userSlScpClassOverview[0].roleStats.deaths/(userSlScpClassOverview[0].roleStats.timePlayed/60)).toFixed(2)}</p>
              ...
              <div className="flex flex-col justify-center items-center border-l-[1px] border-white_opacity">
                <p className="text-[18px] xs:text-[20px]">{(userSlScpClassOverview[0].roleStats.timesJumped/(userSlScpClassOverview[0].roleStats.timePlayed/60)).toFixed(2)}</p>
          ...
          <div className="sm:row-span-2 h-[230px] ss:h-[300px] sm:h-[660px] md:h-[330px] relative bg-dark_opacity sm:bg-transparent">
            <img src="../../../assets/class/SCP-173.svg" alt="" className="absolute bottom-0 right-1/2 translate-x-1/2 h-[210px] ss:h-[280px] sm:h-[420px]  md:h-[340px] " />
          </div>
          <div className=" h-[330px] grid grid-cols-1 grid-rows-2 bg-dark_opacity sm:bg-transparent">
            <div className="flex flex-col justify-center items-center border-b-[1px]  border-white_opacity">
              <p className="text-[48px]">{(userSlScpClassOverview[0].roleStats.timePlayed/60).toFixed(2)}</p>
            ...
            <div className="grid grid-cols-1 grid-rows-1 text-center">
              <div className="flex flex-col justify-center items-center border-white_opacity">
                <p className="text-[48px]">{userSlScpClassOverview[0].roleStats.placedTantrums}</p>
                <p className="text-[24px]">PLACED SUBSTANCE</p>
              </div>
  ...
  export default UserSLScp173ClassSummaryChart
