...
const UsersSLAccuracyTopChart = ({ usersSlOverview, period }: any) => {
  ...
  const sumAccurateShotsCount = (dailyStats: any[]) =>
    dailyStats.reduce((sum, daily) => sum + daily.accurateShots, 0);
  ...
  return (
    <div className='flex flex-col gap-[16px] w-full'>
          ...
          return(
            (
              <div key={user.id} className='flex justify-between border-b-[1px] border-white_opacity pb-[16px] px-6'>
               ...
               <div className='text-[18px] font-roboto font-black'>{user.accurateShots} %</div>
              </div>
            )
          )
      ...
      <div className='flex justify-between text-[16px] ss:text-[20px]'>
        ...
        <button onClick={() => setCurrentPage(currentPage + 1)} disabled={endIndex >= userAccuracyCount.length} className='cursor-pointer hover__text__yellow'>
          Następna Strona
        </button>
      </div>
    </div>
...
export default UsersSLAccuracyTopChart;
