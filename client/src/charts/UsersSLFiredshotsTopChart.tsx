...
const UsersSLFiredshotsTopChart = ({ usersSlOverview, period }: any) => {
  ...
  const userFiredshotsCount = usersSLOverviewArray.map((user: any) => {
    ...
    if(user.nickname == "None") {
        ...
        return {
            ...
            firedshotsCount: sumFiredShotsCount(lastXDayStats),
            number: 0
          };
    } 
  ...
  const sortedUsers = userFiredshotsCount.sort((a: any, b: any) => b.firedshotsCount - a.firedshotsCount);
  ...
  return (
    <div className='flex flex-col gap-[16px] w-full'>
          ...
          return(
            (
              <div key={user.id} className='flex justify-between border-b-[1px] border-white_opacity pb-[16px] px-6'>
               ...
               <div className='text-[18px] font-roboto font-black'>{user.firedshotsCount}</div>
              </div>
            )
          )
      ...
      <div className='flex justify-between text-[16px] ss:text-[20px]'>
        ...
        <button onClick={() => setCurrentPage(currentPage + 1)} disabled={endIndex >= userFiredshotsCount.length} className='cursor-pointer hover__text__yellow'>
          Następna Strona
        </button>
      </div>
    </div>
...
export default UsersSLFiredshotsTopChart;
