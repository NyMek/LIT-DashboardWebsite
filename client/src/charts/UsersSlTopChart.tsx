...
const UsersSLTopChart = ({ usersSlOverview, period, itemsPerPage, sumFunctionCount, text, format  }: any) => {
...
const parseDateStringToDate = (dateString: string): Date | null => {
  ...
  const userCount = usersSLOverviewArray.map((user: any) => {
  ...
  const sortedUsers = userCount.sort((a: any, b: any) => b.count - a.count);
  ...
  return (
    ...
    <div className='flex flex-col gap-[16px] w-full'>
        ...
        paginatedUsers.map((user: any, index:any) => {  
          ...
          return(
            (
                ...
                {
                    format ?
                    <div className='text-[18px] font-roboto font-black'>{((user.count/60)/60).toFixed(2)} h</div>
      ...
      <div className='flex justify-between text-[16px] ss:text-[20px]'>
        ...
        <button onClick={() => setCurrentPage(currentPage + 1)} disabled={endIndex >= userCount.length} className='cursor-pointer hover__text__yellow'>
          Następna Strona
        </button>
      </div>
    </div>
...
export default UsersSLTopChart;
